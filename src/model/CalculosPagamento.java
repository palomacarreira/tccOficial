...
public class CalculosPagamento {
		 ...
		 if(listaDePonto != null)
		 {
			 ...
			 {	 
				...
				if((mesPonto == mesEscolhido) && (anoPonto == anoEscolhido))
				{
					...
					{
						// em caso de compensa��o de horas
						// pagar a mais se no fim do m�s tiver horas sobrando
	...
	public double totalINSS(double salario){
		...
		else if(salario > 5189.82){inss = 570.88;} // teto de contribui��o inss 2016
	...
	public double totalValeTransporte(double salario, String codigoEmpregado)
	{
		...
		double parteSalario = salario * 0.06; // 6% DO SAL�RIO
	...
	{
		// CALCULAR SAL�RIO // SE TIVER HORAS NEGATIVAS DESCONTA
		ContratoTO contratoTO = contrato.pesquisarEmpregado(codigoEmpregado);
	...
	public double totalFolgas(String codigoEmpregado)
	{
		...
		if(totalFolgas > 0){
			//Para calcular o valor basta dividir o sal�rio registrado na carteira profissional 
			//por 30 e multiplicar pelo n�mero de faltas.	
	...
	public double calculaFeriasProporcionais(String codigoEmpregado, double salario, int mesDemissao, int anoDemissao)
	{
		...
		//int diaDiferen�a = ferias.get(Calendar.DAY_OF_MONTH) - atual.get(Calendar.DAY_OF_MONTH);
		int mesDiferenca = (ferias.get(Calendar.MONTH) - atual.get(Calendar.DAY_OF_MONTH)) + 1;
		int anoDiferenca = ferias.get(Calendar.YEAR) - atual.get(Calendar.YEAR);
		int total = anoDiferenca + mesDiferenca;
	...
	public double calculaDecimoPrimeiraParcela(String codigoEmpregado, double salarioBase, int qtdMesesTrabalhados)
	{
	
		double decimoTerceiro = ((salarioBase / 12) * qtdMesesTrabalhados)*2; 
		return decimoTerceiro;
	}
	
	public double calculaDecimoSegundaParcela(String codigoEmpregado,  double salario, int qtdMesesTrabalhados, double inss, double irrf, double valorPrimeiraParcela)
	{
		double decimoTerceiro = ((salario / 12) * qtdMesesTrabalhados); 
		decimoTerceiro = decimoTerceiro - valorPrimeiraParcela - inss - irrf;
		return decimoTerceiro;
	}
}
