...
%>  
	...
	<form id="dadosPonto" role="form" class="form-inline" method="post" action ="AlterarPonto?codigoEmpregado=<%=codigoEmpregado %>&totalDeDias=<%=diasDoMes%>">
...
window.onload = function(){
	 <% 
		...
		if(listaDePonto.size() != 0)
		{
				for(PontoTO pontoTO : listaDePonto)
				{
					...
					// SE TIVER DADOS NO MÊS SELECIONADO PREENCHE
					...
					{ 
						...
						var newRow = $('<tr id=\"linha'+<%=diaPonto%>+'\">');
						var cols = "";
						cols += '<td><p id=\"dia'+ <%=diaPonto%> +'\" name=\"'+diaSemana+'\">'+ <%=diaPonto%>+ '<br>' +diaSemana+'</p></td>';
						cols += '<td><input type= "text" class="hora form-control" name="horaEntrada'+ <%=diaPonto%> +'" id="horaEntrada'+ <%=diaPonto%> +'" size="5" value="<%=pontoTO.getHoraEntrada()%>"></td>';
						cols += '<td><input type= "text" class="hora form-control" name="horaSaidaAlmoco'+ <%=diaPonto%> +'" id="horaSaidaAlmoco'+<%=diaPonto%> +'"size="5" value="<%=pontoTO.getHoraSaidaAlmoco()%>"></td>';
						cols += '<td><input type= "text" class="hora form-control" name="horaVoltaAlmoco'+ <%=diaPonto%> +'" id="horaVoltaAlmoco'+<%=diaPonto%> +'" size="5" value="<%=pontoTO.getHoraVoltaAlmoco()%>"></td>';
						cols += '<td><input type= "text" class="hora form-control" name="horaSaida'+<%=diaPonto%> +'" id="horaSaida'+ <%=diaPonto%> +'" size="5" value="<%=pontoTO.getHoraSaida()%>"></td>';
						cols += '<td>';
						cols += '</td>';
						newRow.append(cols);
						...
						var na = document.getElementById("dia"+<%=diaPonto%>).innerHTML;
						...
						if(nameDia == "SEG")
						{
							...
							var verifica = desabilitaCampos(<%=diaPonto%>, <%=diaNaoTrabalhado%>);
							if(verifica == true){
								adicionaColunaEspecial(<%=diaPonto%>);
							}else{
								if(<%=listaJornada.get(0).getDiaFolga()%> == true){
								adicionaColuna(<%=diaPonto%>);}
								else{ adicionaColunaNormal(<%=diaPonto%>);}
							}
						}
						else if(nameDia == "TER")
						{
						...
						if(<%=pontoTO.getAcao().equals("Dia Comum")%>)
						{
							document.getElementById("combo_acao"+<%=diaPonto%>).selectedIndex = "0";
						}
						else if(<%=pontoTO.getAcao().equals("Falta")%>){
							document.getElementById("combo_acao"+<%=diaPonto%>).selectedIndex = "1";
						}
						else if(<%=pontoTO.getAcao().equals("Falta Justificada")%>){
							document.getElementById("combo_acao"+<%=diaPonto%>).selectedIndex = "2";
						}
						else if(<%=pontoTO.getAcao().equals("Feriado")%>){
							document.getElementById("combo_acao"+<%=diaPonto%>).selectedIndex = "3";
						}
						else if(<%=pontoTO.getAcao().equals("Feriado Trabalhado")%>){
							document.getElementById("combo_acao"+<%=diaPonto%>).selectedIndex = "4";
						}
						else if(<%=pontoTO.getAcao().equals("Dia de DSR")%>){
							document.getElementById("combo_acao_2"+<%=diaPonto%>).selectedIndex = "0";
				  		}
						else if(<%=pontoTO.getAcao().equals("Trabalhou na DSR")%>){
							document.getElementById("combo_acao_2"+<%=diaPonto%>).selectedIndex = "1";
						}
					<%
					}
				}
   ...
   function adicionaColuna(dia)
   {
   		$('#linha'+dia).children('td:last').append($(
   			'<div class=\"form-group\">'+
   			'<select class=\"form-control\" id=\"combo_acao_2'+dia+'\" name=\"acaoSelecionada'+dia+'\">'+
